...
import railview.simulation.container.InfrastructureElementsPane;
...
public class OccupancyAndPendingPaneController {
	...
	public void initialize() {
		...
		pendingBar.setLayoutX(infraRoot.getPrefWidth() / 2
				- pendingBar.getWidth() / 2);
		occupancyBar.setLayoutX(infraRoot.getPrefWidth() / 2
				+ occupancyBar.getWidth() / 2);
	...
	private void mouseEnter() {
		...
		infraRoot.setOnMouseDragged(new EventHandler<MouseEvent>() {
			public void handle(MouseEvent event) {
				infraRoot.setTranslateX(infraRoot.getTranslateX()
						+ event.getX() - pressedX);
				infraRoot.setTranslateY(infraRoot.getTranslateY()
						+ event.getY() - pressedY);
	...
	private void scrollWheel() {
		...
		infraRoot.addEventFilter(ScrollEvent.ANY,
	...
	private void drawTrackLinkWithOPMap(List<Coordinate> coordinates,
			CoordinateMapper mapper, TreeMap<Double, Duration> opMap) {
		...
		for (int i = 0; i < coordinates.size() - 1; i++) {
			totalCoordinateLength += Coordinate.getDistance(coordinates.get(i),
					coordinates.get(i + 1));
		}
		...
		for (Entry<Double, Duration> entry : opMap.entrySet()) {
			...
			List<Coordinate> segement = this.interpolate(coordinates,
					startPercentage, entry.getKey() / totalMeter,
					totalCoordinateLength);
			for (int i = 0; i < segement.size() - 1; i++) {
				Line line = new Line();

				line.setStartX(mapper.mapToPaneX(coordinates.get(i).getX(),
						this.elementPane));
				line.setStartY(mapper.mapToPaneY(coordinates.get(i).getY(),
						this.elementPane));
				line.setEndX(mapper.mapToPaneX(coordinates.get(i + 1).getX(),
						this.elementPane));
				line.setEndY(mapper.mapToPaneY(coordinates.get(i + 1).getY(),
						this.elementPane));
	...
	private List<Coordinate> interpolate(List<Coordinate> coordinates,
			double startPercentage, double endPercentage,
			double totalCoordinateLength) {
		List<Coordinate> segement = new ArrayList<Coordinate>();
		double currentStartPercentage = 0;
		double currentEndPercentage = 0;

		for (int i = 0; i < coordinates.size() - 1; i++) {
			currentEndPercentage = Coordinate.getDistance(coordinates.get(i),
					coordinates.get(i + 1)) / totalCoordinateLength;

			if (segement.size() == 0
					&& startPercentage >= currentStartPercentage
					&& startPercentage < currentEndPercentage) {
				Coordinate coordinate = Coordinate.getCoordinateByPercentage(
						coordinates.get(i),
						coordinates.get(i + 1),
						Percentage.fromDouble(startPercentage
			...
			if (segement.size() > 0) {
				if (endPercentage <= currentEndPercentage) {
					Coordinate coordinate = Coordinate
							.getCoordinateByPercentage(
									coordinates.get(i),
									coordinates.get(i + 1),
									Percentage.fromDouble(endPercentage
											- currentStartPercentage));
					segement.add(coordinate);
					return segement;
				} else {
	...
	private void drawLink(List<Coordinate> coordinates,
			CoordinateMapper mapper, Duration duration) {
		for (int i = 0; i < coordinates.size() - 1; i++) {
			Line line = new Line();
